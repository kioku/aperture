...
#[tokio::test]
async fn test_show_examples_does_not_require_valid_json_body() {
    let mut spec = create_nested_test_spec();
    spec.commands.push(CachedCommand {
        name: "users".to_string(),
        operation_id: "createUser".to_string(),
        summary: Some("Create user".to_string()),
        description: Some("Creates a user".to_string()),
    ...
    let matches = cmd
        ...
        .expect("Should parse command with --show-examples and --body");
    ...
    assert!(
        result.is_ok(),
        "--show-examples should not validate request body JSON: {:?}",
        result.err()
    );
