...
fn test_prompt_for_input_with_control_characters() {
    ...
    let result = prompt_for_input_with_io("Enter text: ", &mock);
    assert!(result.is_err());
    assert!(result.unwrap_err().to_string().contains("invalid control characters"));
}
...
fn test_select_from_options_with_name_selection() {
    let mut mock = MockInputOutputImpl::new();
    let options = vec![
        ("bearerAuth".to_string(), "Bearer token authentication".to_string()),
...
fn test_select_from_options_with_empty_input_and_cancellation() {
    ...
    mock.expect_print()
        .with(eq("Do you want to continue with the current operation? (y/n): "))
        .times(1)
        .returning(|_| Ok(()));
...
fn test_confirm_with_invalid_input_retry() {
    ...
    // Error message
    mock.expect_println()
        .with(eq("Please enter 'y' for yes or 'n' for no. (Attempt 1 of 3)"))
        .times(1)
        .returning(|_| Ok(()));
