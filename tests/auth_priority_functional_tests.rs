use aperture_cli::cache::models::{CachedSpec, CachedCommand, CachedParameter, CachedSecurityScheme, CachedApertureSecret, CACHE_FORMAT_VERSION};
use aperture_cli::config::models::{ApiConfig, ApertureSecret, GlobalConfig, SecretSource};
...
async fn test_missing_config_secret_env_var_error() {
    ...
    assert!(result.is_err(), "Request should fail with missing config secret");
    
    // Verify error message mentions config env var name or secret not set
    let error_message = result.unwrap_err().to_string();
    assert!(
        error_message.contains("CONFIG_BEARER_TOKEN") || error_message.contains("secret not set") || error_message.contains("SecretNotSet"),
...
async fn test_missing_spec_secret_env_var_error() {
    ...
    assert!(result.is_err(), "Request should fail with missing spec secret");
    
    // Verify error message mentions spec env var name or secret not set
    let error_message = result.unwrap_err().to_string();
    assert!(
        error_message.contains("SPEC_BEARER_TOKEN") || error_message.contains("secret not set") || error_message.contains("SecretNotSet"),
...
async fn test_partial_config_override() {
    ...
    assert!(result.is_ok(), "Request should succeed with mixed auth sources");
...
async fn test_no_authentication_configured() {
    ...
    assert!(result.is_ok(), "Request should succeed without authentication");
}
...
async fn test_different_api_configs() {
    ...
    assert!(result.is_ok(), "Request should succeed with spec auth for unmatched API");
